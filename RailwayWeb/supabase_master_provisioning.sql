...
-- MASTER SUPABASE PROVISIONING SCRIPT (SCHEMA + ANALYTICS RPCs)
-- =========================================================================================
...
-- =========================================================================================
-- 10. DASHBOARD ANALYTICS STORED PROCEDURES (RPCs)
-- =========================================================================================

-- RPC 1: CATEGORY DISTRIBUTION (For Pie Charts)
CREATE OR REPLACE FUNCTION get_category_distribution()
RETURNS TABLE (name TEXT, value BIGINT) 
LANGUAGE sql 
AS $$
  SELECT 
    'Grade ' || category AS name, 
    COUNT(*) AS value
  FROM "TEST_ATTEMPT"
  GROUP BY category
...
-- RPC 2: MONTHLY PERFORMANCE TREND (For Line Charts)
CREATE OR REPLACE FUNCTION get_monthly_performance_trend()
...
-- RPC 3: SUBMIT EXAM TRANSACTION (For Assessment Safety & Rollbacks)
CREATE OR REPLACE FUNCTION submit_exam_attempt_transaction(
...
DECLARE
  ...
  ans RECORD;
BEGIN
  -- 1. Insert the master test attempt
  INSERT INTO "TEST_ATTEMPT" (assessment_id, employee_id, total_marks, obtained_marks, percentage, category)
  VALUES (p_assessment_id, p_employee_id, p_total_marks, p_obtained_marks, p_percentage, p_category)
  RETURNING attempt_id INTO v_attempt_id;

  -- 2. Bulk insert answers from JSON array
  FOR ans IN SELECT * FROM jsonb_to_recordset(p_answers) AS x(
    question_id INT, selected_option CHAR, is_correct BOOLEAN, correct_option CHAR, marks_obtained INT
  ) LOOP
    INSERT INTO "ANSWER_HISTORY" (attempt_id, question_id, selected_option, is_correct, correct_option, marks_obtained)
    VALUES (v_attempt_id, ans.question_id, ans.selected_option, ans.is_correct, ans.correct_option, ans.marks_obtained);
  END LOOP;

  -- 3. Update parent assessment to 'Pending'
  UPDATE "ASSESSMENT" SET status = 'Pending' WHERE assessment_id = p_assessment_id;
