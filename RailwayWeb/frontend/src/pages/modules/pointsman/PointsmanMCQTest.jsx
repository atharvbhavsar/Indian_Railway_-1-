...
export function PointsmanMCQTest({
  ...
  pmTestResponses,
  setPmTestResponses,
  pmActiveQIdx: pmActiveQIdxProp, // Avoid collision if needed, but pmActiveQIdx is fine
  setPmActiveQIdx,
  setScreenMode,
  fullName,
  employeeId,
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <div style={{display:"flex", alignItems:"center", gap:12}}>
          ...
          <div>
            <h1 style={{fontSize:18, fontWeight:800, margin:0, color:"#ffffff", letterSpacing:"0.5px"}}>
              {t("POINTSMAN CBT COMPETENCY EVALUATION")}
            </h1>
            <p style={{margin:0, fontSize:11, color:"#94a3b8", fontWeight:500}}>
              {t("Official Railway Safety & Operational Assessment")}
            </p>
          </div>
        </div>
        <div style={{display:"flex", alignItems:"center", gap:16}}>
          <div style={{background:"#334155", padding:"6px 16px", borderRadius:8, fontSize:13, fontWeight:700, color:"#cbd5e1", border:"1px solid #475569"}}>
            ⚙️ {t("Status")}: <span style={{color:"#ea580c"}}>{t("Active Exam Session")}</span>
          </div>
          <button
            onClick={() => {
              if (window.confirm(t("Are you sure you want to abort the exam? Your progress will not be saved."))) {
                if (handleExitExam) {
                  handleExitExam();
                } else {
      ...
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", background:"#ffffff", borderBottom:"1.5px solid #e2e8f0", padding:"12px 24px", height:"50px", flexShrink:0, fontSize:13.5, color:"#334155"}}>
        <div>{t("Candidate")}: <strong style={{color:"#1e3a8a"}}>{fullName}</strong> &nbsp;|&nbsp; {t("HRMS")}: <strong style={{color:"#1e3a8a"}}>{employeeId}</strong> &nbsp;|&nbsp; {t("Station")}: <strong>{t(profile.stationName)}</strong></div>
        <div style={{display:"flex", alignItems:"center", gap:12}}>
          <span style={{fontWeight:600}}>{t("Progress")}: <strong style={{color:"#ea580c"}}>{answeredCount} / 25 {t("Answered")}</strong> ({completionRate}%)</span>
      ...
      <div style={{display:"grid", gridTemplateColumns:"1fr 340px", flex:1, overflow:"hidden"}}>
        ...
        <div style={{padding:"32px 40px", display:"flex", flexDirection:"column", background:"#f8fafc", overflowY:"auto", height:"100%"}}>
          <div style={{background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:14, padding:36, boxShadow:"0 10px 15px -3px rgba(0,0,0,0.05)", flex:1, display:"flex", flexDirection:"column", justifyContent:"space-between", marginBottom:24}}>
            <div>
              <span style={{fontSize:12.5, fontWeight:800, color:"#ea580c", background:"#fff7ed", padding:"6px 14px", borderRadius:20, textTransform:"uppercase", letterSpacing:"0.8px"}}>
                {t("Compulsory Question")} {pmActiveQIdx + 1} {t("of")} 25
              </span>
              <h2 style={{fontSize:22, fontWeight:700, color:"#0f172a", marginTop:24, marginBottom:28, lineHeight:1.5}}>{t(question.text)}</h2>
                ...
                {question.options.map((opt, oi) => {
                  ...
                  return (
                    <label key={oi} style={{display:"flex", alignItems:"center", gap:16, padding:"18px 24px", border:isSelected ? "2.5px solid #ea580c" : "1.5px solid #e2e8f0", borderRadius:12, background:isSelected ? "#fff7ed" : "#ffffff", cursor:"pointer", boxShadow:isSelected ? "0 4px 6px rgba(234,88,12,0.08)" : "none", transition:"all 0.15s ease"}}>
                      ...
                      <span style={{fontSize:15, color:"#1e293b", fontWeight:isSelected?700:500}}>{t(opt)}</span>
                    </label>
                  );
                })}
              </div>
          ...
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:"16px 24px", boxShadow:"0 4px 6px -1px rgba(0,0,0,0.05)"}}>
            <button disabled={pmActiveQIdx===0} onClick={() => setPmActiveQIdx(p=>Math.max(0,p-1))} style={{padding:"12px 28px", borderRadius:8, fontSize:14, fontWeight:700, background:pmActiveQIdx===0?"#f1f5f9":"#ffffff", color:pmActiveQIdx===0?"#94a3b8":"#334155", border:"1.5px solid #cbd5e1", cursor:pmActiveQIdx===0?"not-allowed":"pointer"}}>← {t("Previous Question")}</button>
            <div style={{fontSize:14, color:"#64748b"}}>
              {unansweredCount>0 ? <span style={{color:"#b45309",fontWeight:800}}>⚠️ {unansweredCount} {t("questions remaining")}</span> : <span style={{color:"#16a34a",fontWeight:800}}>✓ {t("All 25 answered! You can submit.")}</span>}
            </div>
            <button disabled={pmActiveQIdx===24} onClick={() => setPmActiveQIdx(p=>Math.min(24,p+1))} style={{padding:"12px 28px", borderRadius:8, fontSize:14, fontWeight:700, background:pmActiveQIdx===24?"#f1f5f9":"#ffffff", color:pmActiveQIdx===24?"#94a3b8":"#334155", border:"1.5px solid #cbd5e1", cursor:pmActiveQIdx===24?"not-allowed":"pointer"}}>{t("Next Question")} →</button>
          </div>
        </div>
        ...
        <div style={{background:"#ffffff", borderLeft:"1.5px solid #e2e8f0", padding:"24px", display:"flex", flexDirection:"column", gap:20, overflowY:"auto", height:"100%"}}>
          <div style={{textAlign:"center", paddingBottom:16, borderBottom:"1.5px solid #f1f5f9"}}>
            ...
            <span style={{fontSize:12, color:"#64748b", fontWeight:500}}>{t("HRMS ID")}: {employeeId}</span>
          </div>
          <h4 style={{fontSize:12, fontWeight:800, color:"#475569", textTransform:"uppercase", letterSpacing:"0.6px", margin:0}}>{t("Question Palette")}</h4>
          ...
          <div style={{borderTop:"1.5px solid #f1f5f9", paddingTop:16, fontSize:12, color:"#64748b", display:"flex", flexDirection:"column", gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:16,height:16,background:"#dcfce7",border:"1.5px solid #86efac",borderRadius:4}}/><span>{t("Attempted")}</span></div>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:16,height:16,background:"#fef3c7",border:"1.5px solid #fde047",borderRadius:4}}/><span style={{color:"#a16207",fontWeight:600}}>{t("Unattempted")}</span></div>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:16,height:16,background:"#ffedd5",border:"2px solid #ea580c",borderRadius:4}}/><span>{t("Current Focus")}</span></div>
          </div>
          <div style={{marginTop:"auto", paddingTop:20, borderTop:"1.5px solid #f1f5f9"}}>
            ...
            >
              {t("Submit Examination")}
            </button>
            {unansweredCount>0 && <p style={{fontSize:11, color:"#b45309", margin:"8px 0 0", textAlign:"center", fontWeight:600}}>* {t("All 25 questions must be answered before submission")} ({unansweredCount} {t("remaining")})</p>}
          </div>
        </div>
      </div>
    </div>
