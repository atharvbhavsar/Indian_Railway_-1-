  ...
  if (!selectedRecord) {
    return (
      <section className="pm-page-card">
        <p className="pm-empty-state">{t("Select a record from History to review.")}</p>
      </section>
    );
  }
  ...
  return (
    <div className="ti2-card animate-fade-in" style={{ padding: "24px", maxHeight: "calc(100vh - 120px)", overflowY: "auto" }}>
      <div className="ti2-card-hdr" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e2e8f0", paddingBottom: "14px", marginBottom: "20px" }}>
        ...
        <button className="ti2-primary-btn" onClick={() => setScreenMode("default")}>{t("← Return to History")}</button>
      </div>
      ...
      <div className="pm-scorecard-hero" style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "20px", display: "flex", alignItems: "center", gap: "24px", marginBottom: "24px" }}>
        ...
        <div>
          ...
          <p className="pm-sc-period" style={{ margin: "2px 0", fontSize: "14px", color: "#1e293b", fontWeight: "600" }}>{t(selectedRecord.assessmentPeriod)} - {t("Self-Compliance Audit")}</p>
          <p className="pm-sc-date" style={{ margin: "2px 0 0", fontSize: "12px", color: "#64748b" }}>{t("Attempt Completed:")} {selectedRecord.date} &nbsp;·&nbsp; {t("Assessed By:")} {t(selectedRecord.assessedBy || "S. Deshmukh (Station Master)")}</p>
        </div>
      </div>
      ...
      <div className="pm-performance-summary-box" style={{ background: "#eff6ff", borderLeft: "4px solid #2563eb", padding: "16px", borderRadius: "8px", marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: "#1e3a8a", lineHeight: "1.5" }}>{t(performanceSummaryText)}</p>
      </div>
